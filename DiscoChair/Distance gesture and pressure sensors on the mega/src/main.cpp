...
******************************
*/
#define IR_PIN  A5                       // Setting up pin to receive voltage from IR

int value_Distance, prev_value_Distance = - 10000;         // int values (read from analog port, both the current and the previous)
int deviationDistance = 0;                       // setting the minimum deviation between the measurements (0 by default)
                                         // up to 512 (although that is pretty useless)
double voltage_value_Distance, distance_value_Distance;    // Converted to Voltage


// insert Proximity varriables here
String distanceID = "";

double convert_to_distance( double voltage)
{
    if(voltage_value_Distance  < 0.35  || voltage_value_Distance > 2.85 ) // We will ignore values outside the range of measurement, this will happen around 20 - 150cm
    return(0);
...
void distance()
{
  ...
  value_Distance = analogRead(IR_PIN);       // reading our analog voltage, careful we only have 10 bit resolution so each
                                    // measurement step is only 5V ÷ 1024, so our result will be 0 - 1023

  // if value is within the range of [ previous - σ , previous + σ], ignore it (if value is relatively the same)
  // this will help with having data ocuppy your buffer that is not a significant deviation.
  if( value_Distance >= (prev_value_Distance - deviationDistance) && value_Distance <= (prev_value_Distance + deviationDistance) )
    return;

  voltage_value_Distance = double((value_Distance*5)) / 1023; // converting to voltage [ 0, 5] v.

  distance_value_Distance = convert_to_distance(voltage_value_Distance); // getting actual distance value(cm) (careful using this, accuracy may not be ideal)
                                                       // due to the functioning of the sensor, once you're closer than around 20 cm, it will
                                                       // start predicting higher distances again. Be careful with this, this is something you can
                                                       // solve with software, however. (if previous results are close to 20 and its going down)
                                                       // then do something.... to ignore results, perhaps.

    if(distance_value_Distance  < 20  || distance_value_Distance > 150 ) // We will ignore values outside the range of measurement, this will happen around 2.7 -0.4 v
    return;

  Serial.print("Distance: ");
  Serial.print(value_Distance);
  Serial.print(" (0 - 1023) steps,  ");
  Serial.print(voltage_value_Distance);
  Serial.print(" (v),  ");
  Serial.print(distance_value_Distance);
  Serial.println(" cm.");

  prev_value_Distance = value_Distance;             // Here we have the previous saved variable.
